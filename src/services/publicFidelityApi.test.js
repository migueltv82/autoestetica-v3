...
describe("publicFidelityApi", () => {
  ...
  it("consulta por token privado sin enviar solamente el teléfono", async () => {
    supabase.rpc.mockResolvedValue({ data: [{ card_id: "card-1", public_token: "token-1", client_name: "Ana", stamps_count: 2, total_stamps: 4, status: "active", vehicle_label: "Ford Ka" }], error: null });
    ...
    expect(result).toMatchObject({ id: "card-1", publicToken: "token-1", clientName: "Ana", stampsCount: 2, vehicle: "Ford Ka" });
  });
