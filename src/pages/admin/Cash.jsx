...
function Cash() {
  ...
  return <PageTransition><AdminLayout title="Control de caja" subtitle="Cobros, gastos, saldos y recibos del negocio.">
    ...
    {!isLoading && transactions.length ? <section className="cash-movement-list" aria-label="Movimientos de caja">{transactions.map((item) => <article key={item.id} className={item.type}><span className="cash-flow-icon">{item.type === "income" ? <ArrowUpRight size={18} /> : <ArrowDownRight size={18} />}</span><div className="cash-movement-main"><strong>{item.description}</strong><small>{item.date} · {item.category || "Sin categoría"}</small></div><span className="cash-method"><MethodIcon method={item.method} /> {item.method}</span><strong className="cash-amount">{item.type === "income" ? "+" : "−"}{money(item.amount)}</strong><div className="cash-row-actions"><button type="button" onClick={() => handleEdit(item)} aria-label="Editar movimiento"><Edit2 size={15} /></button><button type="button" className="danger" onClick={() => handleDelete(item.id)} aria-label="Eliminar movimiento"><Trash2 size={15} /></button></div></article>)}</section> : null}
