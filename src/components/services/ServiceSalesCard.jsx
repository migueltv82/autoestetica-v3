...
function Price({ service }) {
  ...
  if (isTwoWheelService(service)) return <div className="sales-card-prices"><small>Precio del servicio</small><strong>{formatMoney(service.carPrice || 0)}</strong></div>;
  return <div className="sales-card-prices two"><span><small>Auto</small><strong>{formatMoney(service.carPrice || 0)}</strong></span><span><small>Camioneta</small><strong>{formatMoney(service.truckPrice || 0)}</strong></span></div>;
}

export default function ServiceSalesCard({ service, whatsappNumber, compact = false, showFeaturedBadge = true }) {
