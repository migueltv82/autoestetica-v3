...
function TurnForm({ onAddTurn, initialData = null }) {
  ...
  function toggleService(service) {
    setFormData((current) => {
      ...
      return {
        ...current,
        services: selected
          ? current.services.filter((item) => item.serviceId !== service.id)
          : [...current.services, {
  ...
  function updatePrice(serviceId, price) {
    setFormData((current) => ({
      ...current,
      services: current.services.map((service) => service.serviceId === serviceId ? { ...service, price } : service),
    }));
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      await onAddTurn({
        ...formData,
        registerPayment: canManageFinance && formData.registerPayment,
        amount: total,
        durationMinutes,
      });
      setFormData(buildInitialForm(canManageFinance));
    } catch {
      // Conservar datos cargados para que el usuario pueda corregir.
    }
    ...
    <div className="admin-form-shell turn-form-shell">
      <div className="admin-form-header">
        <div>
          ...
          <p className="admin-form-description">
            {canManageFinance
              ? "Al guardar, el cliente, los servicios y el cobro quedan relacionados automáticamente."
              : "Al guardar, el cliente, el vehículo y los servicios quedan relacionados automáticamente."}
          </p>
        </div>
      </div>
      ...
      <form className="turn-form" onSubmit={handleSubmit}>
        <div className="admin-form-grid wide turn-form-grid">
          ...
          <div className="admin-form-group">
            <label><Car size={14} /> Vehículo</label>
          ...
          {canManageFinance ? (
            <>
              <div className="admin-form-group">
                <label><CreditCard size={14} /> Medio de pago</label>
              ...
              <label className="turn-payment-toggle">
                <input type="checkbox" checked={formData.registerPayment} onChange={(event) => setFormData((current) => ({ ...current, registerPayment: event.target.checked }))} />
                <span><Wallet size={17} /><strong>Registrar el cobro en caja</strong><small>Desmarcalo si el cliente todavía no pagó.</small></span>
              </label>
            </>
          ) : null}
