...
import { getTurnWhatsAppLink } from "../../utils/whatsapp";
...
export default function SimpleAgenda({ turns, selectedDate, onDateChange, search, onSearchChange, statusFilter, onStatusFilterChange, onStatusChange, onEditTurn, onDeleteTurn, onGenerateReceipt, onViewTurn, settings, canUseOperationalActions = true }) {
  ...
  return <div className="simple-agenda">
    ...
    <div className="agenda-quick-summary">
      <span><strong>{visibleTurns.length}</strong> turnos</span><span><strong>{activeCount}</strong> activos</span><span><strong>{formatMoney(total)}</strong> total</span>
    </div>
    ...
    <section className="agenda-card-list">
      {visibleTurns.map((turn) => <article className={`agenda-mobile-turn is-overlapping ${getServiceTone(turn)} status-${getTurnStatusClass(turn.status)}`} key={turn.id} {...getTurnCardInteractionProps(turn, onViewTurn)}>
        ...
        {turn.amount > 0 ? <strong className="agenda-turn-price">{formatMoney(turn.amount)}</strong> : null}
        {canUseOperationalActions ? <div className="agenda-primary-action">{turn.phone && !["Cancelado", "Finalizado"].includes(turn.status) ? <a href={getTurnWhatsAppLink(turn, settings)} target="_blank" rel="noreferrer"><MessageCircle size={18} /> {turn.status === "Listo" ? "Avisar vehículo listo" : "Confirmar turno"}</a> : <span>Sin acción de WhatsApp</span>}</div> : null}
