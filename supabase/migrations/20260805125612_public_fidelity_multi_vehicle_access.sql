-- Un acceso privado valido habilita todas las tarjetas activas del mismo cliente.
-- Esto permite recordar el acceso en el dispositivo y cambiar entre vehiculos.
drop function if exists public.lookup_public_fidelity_card(text,text,text);

create function public.lookup_public_fidelity_card(
...
begin
  if nullif(trim(coalesce(p_token,'')),'') is null then
    ...
    select fc.client_id into authorized_client
    ...
    from public.fidelity_cards fc
    join public.clients c on c.id=fc.client_id
    where fc.public_token::text=lower(trim(p_token))
  ...
  return query
  select fc.id,fc.public_token,c.name::text,fc.stamps_count,fc.total_stamps,fc.status::text,
    fc.total_rewards_redeemed,
    coalesce(nullif(trim(concat_ws(' ',v.brand,v.model)),''),v.type,'Vehiculo')::text
  from public.fidelity_cards fc
  join public.clients c on c.id=fc.client_id and c.deleted_at is null
